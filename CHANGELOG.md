...
- Treated MySQL `#` line comments as comments. A `where` inside one no longer hides a missing `WHERE` from the full-table `UPDATE`/`DELETE` confirmation, a semicolon inside one no longer splits the statement, and one after the last semicolon no longer runs as a statement of its own.
