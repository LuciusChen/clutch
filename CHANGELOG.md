...
- Made row identity resolution cheaper and visible. It runs synchronously before every statement: a table without a primary key previously enumerated every index in the schema on each query (seconds on large Oracle schemas) and nothing cached the answer. The lookup now asks for that table's indexes only, the result is reused per relation until schema refresh, DDL, reconnect or a schema switch discards it (failed lookups are not cached), and `clutch-debug-mode` records each resolution with its table, chosen candidate and duration. A table without a unique index also no longer fetches column details on that path, which was three quarters of the remaining first-resolution cost.
