...
- **SAP HANA JDBC backend** (`saphana`): Core SQL support through SAP's official `ngdbc` driver from Maven Central (`clutch-jdbc-install-driver 'saphana`).  Includes `?currentSchema=` URL mapping, generous default RPC/read-idle timeouts for `_SYS_BIC` calculation-view metadata, lazy schema enumeration for large tenants, and auto-commit by default (manual transactions require an explicit `:manual-commit t`).  See `docs/saphana-backend.org`.
- SAP HANA `.hana.gpg` auth-source discovery: entries in `clutch-saphana-auth-source-files` (default `~/.hana.gpg`) surface as virtual saved connections in the `clutch-query-console` picker alongside `clutch-connection-alist`, with static names winning on collision.  New `clutch-saphana-*` customization options.
- Runtime schema switching (`clutch-switch-schema`) for SAP HANA: the picker lists user and application schemas from a bounded `SYS.SCHEMAS` query (system and `_SYS_*` schemas hidden), and selecting one updates the effective `currentSchema`.
- Query consoles bound to a saved connection now refuse to silently reconnect when that saved name resolves to a different endpoint than the one the console was opened for (e.g. a static `clutch-connection-alist` entry is removed and a discovered `.hana.gpg` entry reuses its name).  A `user-error` is raised instead of quietly rerouting to another database.  Backend-agnostic.
...
- Fixed SQL column completion for quoted identifiers containing dots or `::`, such as SAP HANA calculation-view names (`"com.sap.app::Foo.Bar"`).  The statement parser now captures such names whole (no longer truncating at `::`) and the table-token normalizer splits only on unquoted dots, so completion resolves the real table instead of a mangled suffix.
