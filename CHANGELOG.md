...
- Stopped Emacs from stalling after a long SQL paste. Eldoc, completion and the other context features find the statement at point by scanning the whole buffer for top-level semicolons from Lisp, one callback per character, and then masked the statement the same way, so on a 4 MiB paste the first Eldoc hint on a trailing identifier took about 0.5 s, `completion-at-point` after the terminating semicolon about 1.7 s, and with automatic completion every keystroke paid it again. The scanner now jumps between literals, comments, parentheses and the characters a caller asked for with a regexp search, the scan is cached per buffer change and reused up to the edited statement, and completion stops at a statement's terminating semicolon instead of offering the previous statement's columns. The same 4 MiB paste now takes 63 ms for that hint, 29 ms for that completion, and 3 ms per keystroke in a script; a single 4 MiB statement still rescans on each keystroke, in about 60 ms. Execution splits statements with the same scanner, so `clutch-execute-buffer` on that paste prepares its statements in 33 ms instead of 290 ms. Every dialect rule is unchanged: strings, `--` and `/* */` comments, quoted identifiers, PostgreSQL dollar quotes and MySQL backslash escapes split statements exactly as before.
...
- Kept the schema listing off describe and the object warmup. Describing a table read a full listing of the schema's browseable objects twice (to find its indexes, then its triggers) and each background warmup category listed it again; on a 32,780-table Oracle schema every listing cost 1.3 s, so a describe spent over 3 s before fetching its own metadata and paid it again right after connecting. Both now read only the warmed categories, and on Oracle the listing the object picker still needs is kept for one schema cache generation; other backends keep listing it on each use, because their objects also change through ordinary commands such as Redis writes. Resolving the object at point on a backend that searches by name (Oracle) stopped listing the schema too: it uses the prefix search plus the warmed categories, so a describe two seconds after connecting resolves in half a second instead of ten.
...
- Matched SQL clause keywords the same way from every buffer, and never inside a name that contains `_`, `#` or `@`. From a SQL buffer, a clause split across lines (`ORDER` / `BY`) and a semicolon followed by a newline went unseen: SQL Server paging appended a second `ORDER BY`, and `DELETE ... WHERE 1 = 1` followed by such a clause skipped its `YES` confirmation. From any buffer, columns such as `valid_from`, `group_id` and `credit_limit` matched the keyword after the underscore, which took `FROM` for the source table, turned off row editing and server-side rewrites, or ran the query without a row limit. An Oracle name such as `valid#where` or a SQL Server variable such as `@where` also hid a missing `WHERE` from the full-table `UPDATE`/`DELETE` confirmation.
- Left hidden row identity columns out of a select list that contains a comment or optimizer hint, and qualified a lowercase `select *` whatever `case-fold-search` is. Columns appended after a trailing `--` comment were commented out along with `FROM`, and Oracle rejects extra columns after `SELECT /*+ hint */ *` or an unqualified lowercase `select *` (ORA-00923).
- Kept the clauses that paging, sorting, counting and filtering add to a statement out of its trailing comment. A `SELECT` whose last line ended in a `--` comment failed on Oracle with ORA-00907 (missing right parenthesis); on other backends it fetched the whole table with every page starting at the first row, and its row count and filters failed.
- Listed a table's indexes and triggers in describe on JDBC connections before the object warmup has loaded them, by asking the agent for that table's objects only. On Oracle, whose warmup takes seconds on a large schema, both sections were silently left out until it finished.
- Ran a region or buffer that ends with a comment after its last semicolon, instead of running the comment as a statement of its own and failing with "Statement 2 failed".
...
- Pinned the published clutch-jdbc-agent 0.2.24, which runs Oracle schema-wide listings (`get-tables`, `get-sequences`, `get-procedures`, `get-functions`, and `get-indexes`/`get-triggers` without a table) on a third, lazily opened session. Queries issued while the background object warmup lists an Oracle schema no longer wait for that listing to release the metadata session before their row identity lookup can run; an account that cannot open a third session keeps listing on the metadata session, and other products keep two sessions and an unchanged request path. The agent also validates an idle metadata or bulk session before using it, after `clutch-jdbc-validate-after-idle-seconds`: a session silently dropped by a NAT or firewall made the next metadata request wait 30 seconds and then end the whole connection, losing an uncommitted transaction.
- Pinned the published clutch-jdbc-agent 0.2.25. A schema switch on Oracle now closes the cursors of the listing session it drops, so a listing paged across the switch reports an unknown cursor instead of reaching the closed session, and when a metadata or listing session fails the agent forgets all of its cursors without calling into the dead driver.
