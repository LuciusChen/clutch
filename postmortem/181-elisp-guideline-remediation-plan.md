...
- Phase 4 converted only indentation tabs in executable forms and retained tabs
  that align command/help columns inside docstrings. Six equivalent
  `when`/`not` guards now use `unless`, and three condition-only `not`/`null`
  coercions now use ordinary Elisp truthiness. Static rescans and
  `git diff --check` are clean. The complete non-live gate passes with 544 main,
