...
The migration was developed on `refactor/pgsql`, branched from Clutch main while main retained the released PostgreSQL path. Integration CI installs `pgsql` from MELPA and does not install `pg-el`, so an accidental old-API dependency fails rather than hiding behind the developer's package state.
...
- Installation, CI package setup, upgrade, and rollback instructions match the code, and the old pg-el workarounds are removed instead of retained as dormant alternatives.
