...
Matching the symbol at point (`clutch--object-matches-by-name`) listed every table-like object and then filtered for one exact name. On the same Oracle schema that listing is 33,460 entries over 66 cursor round trips: one `clutch-jump` measured 1.8 s, almost all of it in `clutch-db-list-table-entries`, and a second invocation cost the same because the listing is not cached. The review had predicted three catalog fetches per command; measurement showed one — the first match short-circuits the rest — so the cost was the size of that one listing, not its repetition.

Postmortem 037 already routes Oracle completion through `search-tables` because its schema enumeration is too slow to block on. The at-point match now asks that prefix search for the name it needs, gated by a backend capability, `clutch-db-object-name-search-p`, which only the JDBC backend answers for Oracle. Gating on `clutch-db-eager-schema-refresh-p` was tried first and was wrong: native MySQL and PostgreSQL also defer schema refresh, but they have no search of their own, and the default search derived from completion returns entries without a schema and with views typed as tables. The capability names the real contract — the search must return what the listing would for that name — rather than a property that happens to correlate with it on one backend. Measured on the reporter's database: 77 ms, then 58 ms, resolving the same entry. The fallback that offers the full picker when nothing matches still lists, because that is what the picker shows.
