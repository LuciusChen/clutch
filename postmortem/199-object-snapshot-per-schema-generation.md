...
Resolving the object at point for `clutch-describe-dwim` went through the same listing. With any object type allowed, `clutch--object-matches-by-name` built the full snapshot rather than the prefix search that its table-only branch already used. On an unshared connection two seconds after connecting, with the index warmup in flight, that resolve took 10.1 s: the snapshot's `get-tables` queued behind the 8 s `get-indexes` on the bulk lane, then listed 32,780 tables. The table-only resolve took 0.5 s at the same moment and 60 ms once warm.

## Decision

Describe and the warmup do not need the snapshot. The related-index and related-trigger lookups read only the warmed categories, which is all they ever matched (a table entry is never an index or a trigger); when a category is not warmed yet they return nothing and schedule the warmup, as the partial snapshot already did. The warmup's category store no longer merges the table snapshot into the object cache; the only reader of table entries there, the table-type lookup, now takes them from the cached snapshot instead. So neither path lists the schema, cold or warm, and a describe right after connecting costs its own metadata only. Resolving the object at point follows the same rule on a backend that searches by name: table-like matches come from the prefix search and the other types from the warmed categories, so an index name resolves once the warmup has loaded indexes and, until then, the reader falls back to the picker as before.

The snapshot still exists for the picker and browser fallbacks that show tables. On a backend that searches objects by name (Oracle), `clutch--browseable-object-entries` lists it once per schema cache generation, primes table comments from it (the category store used to do that on every step), and drops it where the object cache is already dropped, when the schema cache reports `invalidated` (a refresh installing new names, a reconnect, a schema switch, cleared metadata). An explicit object refresh lists again and replaces the cached snapshot, so the browser's refresh still reaches the database.

Nothing changes in how any backend lists objects.

## Other backends

Only name-search backends keep the snapshot between calls. Everywhere else it is listed on each use, as before, because those objects also change through commands that never refresh the schema cache: a Redis `SET` or `DEL`, an insert that creates a MongoDB collection, DDL on an eager-refresh backend. Caching them froze the picker on the first listing. Taking the snapshot out of describe and the warmup applies to every backend; only the magnitude differs. Every JDBC product lists the whole schema per snapshot, so SQL Server, DB2, Snowflake, DuckDB and ClickHouse pay one listing per describe and per warmup step in proportion to their table count and latency. MySQL, PostgreSQL and SQLite run two statements per snapshot through the default method. None of them showed seconds, because their schemas in use are small and local, but the fix is in the shared object layer and applies to all of them without touching a backend.
...
On the reporting database a table describe pays no schema listing at any point in the session, only its own metadata (about 0.35 s there); its Indexes and Triggers sections appear once the warmup has loaded those categories, as before. The warmup no longer lists the schema at all; the object picker's full-listing fallback lists once per schema generation. In the first seconds after connecting that one listing still queues behind the index warmup on the bulk lane, so a picker opened that early waits up to about ten seconds once. This is deferred: it needs the picker before the warmup finishes, which is not how the connection is used. If it becomes worth it, the schema refresh should hand its full listing to the object layer instead of keeping only the names, which would also remove the second listing per generation. The snapshot is as fresh as the schema cache: an object created outside clutch appears after the next schema refresh, which is already the rule for completion.
