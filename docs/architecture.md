...
`clutch-db-saphana.el` is a SAP HANA extension layered on top of the `clutch-db-jdbc.el` adapter, not a standalone backend. HANA-specific JDBC behavior (`currentSchema` URL mapping, timeout defaults, lazy schema enumeration, object-category skip-list, `SET SCHEMA` switching, row-identity policy) lives in `clutch-db-jdbc.el`; `clutch-db-saphana.el` owns only `.hana.gpg` auth-source discovery. It registers that discovery through `clutch-external-connection-source-functions`, an inversion-of-control hook owned by `clutch-connection.el`. The hook exists precisely so `clutch-connection` can surface discovered connections without depending on any adapter module (which the layer boundary forbids); it is the seam a future backend would reuse to contribute virtual saved connections. Because a discovered auth-source entry can reuse the display name of a since-removed static entry, `clutch--connect-params-for-current-buffer` records each console's opened-for endpoint fingerprint and refuses to silently reconnect a console whose saved name now resolves to a different endpoint.
