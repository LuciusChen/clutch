...
TLS can be enabled per entry with =:tls t=; by default TLS verifies both
certificate chain and hostname.  For private PKI, set
=mysql-tls-trustfiles= / =pg-tls-trustfiles= to CA files.
