...
| PostgreSQL | Core SQL support | Requires current [[https://github.com/emarsden/pg-el][pg-el]] with =pgcon-transaction-status= |
...
JDBC support covers Oracle, SQL Server, SAP HANA, DB2, Snowflake, Redshift, ClickHouse, MongoDB SQL Interface, DuckDB, and generic JDBC URLs through the [[https://github.com/LuciusChen/clutch-jdbc-agent][clutch-jdbc-agent]] sidecar.  For setup, driver installation, connection examples, backend-specific notes, and transaction behavior, see [[file:docs/jdbc-backend.org][docs/jdbc-backend.org]].  SAP HANA also has a dedicated guide covering =currentSchema= mapping and =.hana.gpg= auth-source discovery in [[file:docs/saphana-backend.org][docs/saphana-backend.org]].
