...
When =auth-source-pass= is loaded, clutch automatically tries a pass
entry named after the connection (the key in =clutch-connection-alist=).
...
Store the password (first line of the entry):

#+begin_src sh
pass insert dev-mysql
#+end_src

The pass store contains only =dev-mysql= — no host or username in the
file path.  Use =:pass-entry "other-name"= to override the entry name
when it must differ from the connection name.
