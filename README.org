...
- WHERE filtering and SQL =ORDER BY= sorting
...
| =C-c P=     | 取消固定                         |
| =W=         | WHERE 过滤（再按清除）           |
| =s= / =S=  | 按列排序 ASC / DESC（SQL ORDER BY） |
| =c=         | 跳转到某列                       |
| =g=         | 重新执行查询                     |
| =e=         | 编辑 cell 值                     |
...
2. C-c C-c  执行 → 结果分页显示在底部（默认每页 500 行）
3. n / p    翻到下一页 / 上一页
4. #        查询总行数 → footer 显示 "rows 1-500 of 1234"
5. W        输入过滤条件: status = 'pending'
            → 自动追加 WHERE 重新查询，从第一页开始
6. s        按当前列排序 (SQL ORDER BY)，从第一页开始
7. W        输入空字符串 → 清除过滤，恢复原始查询
#+end_src
