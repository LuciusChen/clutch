...
*** pass example — opaque entry name (recommended)

Use =:pass-entry= to name the pass entry whatever you like, keeping
host and user off the filesystem:
...
(setq clutch-connection-alist
      '(("zj_test@225" . (:backend mysql
                           :host "192.168.1.225" :port 3306
                           :user "cjh_test_225" :database "zj_test"
                           :pass-entry "zj_test@225"))))
#+end_src

Store the password (first line only):

#+begin_src sh
pass insert zj_test@225
#+end_src

clutch looks up the entry by name directly, so the pass store reveals
only =zj_test@225= — no IP address or username in the file path.
