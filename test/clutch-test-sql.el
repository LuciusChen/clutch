  ...
  (clutch-test--with-isolated-metadata-caches
    (let* ((conn 'fake-conn)
      ...
      (with-temp-buffer
        ...
        (insert "select * from typo_table where nam")
        (goto-char (point-min))
        (search-forward "nam")
        (cl-letf (((symbol-function 'clutch-db-completion-deferred-columns-p)
                   (lambda (_conn) t))
                  ((symbol-function 'clutch-db-live-p)
                   (lambda (_conn) t))
                  ((symbol-function 'clutch-db-busy-p)
                  ...
                  ((symbol-function 'clutch-db-list-columns-async)
                   (lambda (_conn table callback &optional _errback)
                     (should (equal table "typo_table"))
                     (funcall callback nil)
                     'fake-timer)))
          (completion-at-point)
          (should (eq (gethash "typo_table" schema missing) missing)))))))

(ert-deftest clutch-test-native-sql-capf-does-not-retry-failed-column-load ()
  "Native SQL CAPF should leave failed metadata idle until schema refresh."
  (clutch-test--with-isolated-metadata-caches
    (let* ((conn 'fake-conn)
           (schema (make-hash-table :test 'equal))
           (requests 0))
      ...
      (with-temp-buffer
        (clutch-mode)
        (setq-local clutch-connection conn)
        (insert "select * from users where nam")
        (goto-char (point-min))
        (search-forward "nam")
        (cl-letf (((symbol-function 'clutch-db-completion-deferred-columns-p)
                   (lambda (_conn) t))
                  ...
                  ((symbol-function 'clutch-db-busy-p)
                   (lambda (_conn) nil))
                  ((symbol-function 'clutch-db-list-columns-async)
                   (lambda (_conn table _callback &optional errback)
                     (should (equal table "users"))
                     (cl-incf requests)
                     (funcall errback "permission denied")
                     'fake-timer)))
          (completion-at-point)
          (completion-at-point)
          (should (= requests 1)))))))
