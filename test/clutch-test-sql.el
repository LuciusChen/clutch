  ...
  (with-temp-buffer
    (let ((clutch-connection 'fake-conn)
      ...
      (should (equal (nreverse calls)
                     '(("UPDATE demo SET seen = 1 WHERE id = 1" nil nil)
                       ("SELECT * FROM demo" t t))))
...
(ert-deftest clutch-test-statement-table-identifiers-handle-hana-quoted-names ()
  "SAP HANA quoted names with dots and `::' must be captured and kept whole.
The table-name scanner must not stop at `::' or split the quoted identifier
on its embedded dots, otherwise column completion looks up a truncated name."
  (should (equal (clutch--statement-table-identifiers-in-sql
                  "SELECT * FROM \"com.sap.ems.db.common.data::AsyncProcess.Monitor\" m")
                 '("com.sap.ems.db.common.data::AsyncProcess.Monitor")))
  (should (equal (clutch--statement-table-identifiers-in-sql
                  "SELECT * FROM \"MY_SCHEMA\".\"com.sap.ems::Foo.Bar\"")
                 '("com.sap.ems::Foo.Bar")))
  ;; Ordinary names and PostgreSQL casts in the body stay unaffected.
  (should (equal (clutch--statement-table-identifiers-in-sql
                  "SELECT ts::date FROM events WHERE flag::int = 1")
                 '("events"))))

(ert-deftest clutch-test-normalize-statement-table-token-keeps-quoted-dots ()
  "Token normalization must split only on unquoted dots.
A quoted identifier such as SAP HANA's
\"com.sap.app::Foo.Bar\" legitimately contains dots and must survive whole,
otherwise column completion looks up the wrong (truncated) table name."
  ;; Classic schema-qualified names still reduce to the bare table.
  (should (equal (clutch--normalize-statement-table-token "\"HR\".\"EMPLOYEES\"")
                 "EMPLOYEES"))
  (should (equal (clutch--normalize-statement-table-token "myschema.mytable")
                 "mytable"))
  ...
  (should (equal (clutch--normalize-statement-table-token "employees")
                 "employees"))
  ;; HANA: a single quoted name whose bytes include dots and `::'.
  (should (equal (clutch--normalize-statement-table-token
                  "\"com.sap.ems.db.common.data::AsyncProcess.Monitor\"")
                 "com.sap.ems.db.common.data::AsyncProcess.Monitor"))
  ;; HANA schema-qualified: "SCHEMA"."dotted.table" keeps only the quoted
  ;; table segment, dots intact.
  (should (equal (clutch--normalize-statement-table-token
                  "\"MY_SCHEMA\".\"com.sap.ems::Foo.Bar\"")
                 "com.sap.ems::Foo.Bar"))
  (should (null (clutch--normalize-statement-table-token nil))))
