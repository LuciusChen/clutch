...
(ert-deftest clutch-architecture-top-level-requires-cover-variable-declarations ()
  "Only real definitions in prior mandatory owners cover bare `defvar' forms."
  (let* ((source-forms
          '((require 'clutch-eager)
            (defvar eager-local)
            (defvar eager-option)
            (defvar eager-mode)
            (defvar eager-constant)
            (defvar source-fallback nil)
            (defvar fallback-state)
            (require 'clutch-fallback)
            (defvar fallback-state)
            (require 'clutch-optional nil t)
            (defvar optional-state)
            (when enabled (require 'clutch-lazy))
            (defvar lazy-state)
            '(require 'clutch-quoted)
            (defvar quoted-state)
            (defvar late-state)
            (require 'clutch-late)
            (require 'clutch-pure-owner)
            (defvar pure-owner-state)
            (require 'clutch-inert-owner)
            (defvar inert-owner-state)))
         (parsed
          `(("clutch-source" . ,source-forms)
            ("clutch-eager"
             (defvar-local eager-local nil)
             (defcustom eager-option nil "Option." :type 'boolean)
             (define-minor-mode eager-mode "Mode.")
             (defconst eager-constant 1)
             (defvar source-fallback t))
            ("clutch-fallback" (defvar fallback-state nil "State."))
            ("clutch-optional" (defvar optional-state nil "State."))
            ("clutch-lazy" (defvar lazy-state nil "State."))
            ("clutch-quoted" (defvar quoted-state nil "State."))
            ("clutch-late" (defvar late-state nil "State."))
            ("clutch-pure-owner" (defvar pure-owner-state))
            ("clutch-inert-owner" '(defvar inert-owner-state nil "State.")))))
    (should
     (equal
      (clutch--architecture-redundant-variable-declarations parsed)
      '(("clutch-source" "clutch-eager" eager-local)
        ("clutch-source" "clutch-eager" eager-option)
        ("clutch-source" "clutch-eager" eager-mode)
        ("clutch-source" "clutch-eager" eager-constant)
        ("clutch-source" "clutch-fallback" fallback-state))))))
