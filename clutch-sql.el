...
Only unquoted dots separate qualifier segments: a quoted identifier such as
SAP HANA's \"com.sap.app::Foo.Bar\" legitimately contains dots and must be
kept whole, so splitting happens outside quotes/backticks and the surrounding
quotes are stripped from the final segment."
  (when token
    (let ((parts nil)
          (start 0)
          (i 0)
      ...
      (while (< i len)
        (let ((ch (aref token i)))
          (cond
           (in-quote
            (when (eq ch quote-char)
              (setq in-quote nil quote-char nil)))
           ((or (eq ch ?\") (eq ch ?`))
            (setq in-quote t quote-char ch))
           ((eq ch ?.)
            (push (substring token start i) parts)
            (setq start (1+ i)))))
        (setq i (1+ i)))
      (push (substring token start) parts)
      (replace-regexp-in-string "[\"`]" "" (car parts)))))
